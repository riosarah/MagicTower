...
namespace MagicTower.Logic.Modules.CodeGenerator
{
    /// <summary>
        ...
        public static Type[] AllTypes
        {
            get
            {
                var allTypes = AppDomain.CurrentDomain
                                        .GetAssemblies()
                                        .Where(a => a.FullName!.StartsWith($"{nameof(MagicTower)}{Common.StaticLiterals.CommonExtension}")
                                                 || a.FullName!.StartsWith($"{nameof(MagicTower)}{Common.StaticLiterals.LogicExtension}"))
                                        .SelectMany(assembly =>
                                        {
                                            try
                                            {
